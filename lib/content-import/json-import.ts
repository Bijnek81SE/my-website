...
export function importJsonRecords(source: string): ImportResult<ImportedRecord> {
  try {
    ...
    const diagnostics = records.length === values.length
      ? []
      : [{ level: "error" as const, code: "non-object-record", message: "JSON imports must contain objects or an array of objects." }];
