...
export default function WorkspaceToolbar() {
  const { snapshot, canUndo, setMolecule, undo, reset } = useWorkspace();
  ...
  return (
    <div className="flex flex-col gap-4 border-b border-slate-200 bg-white p-4 sm:flex-row sm:items-end sm:justify-between">
      <label className="block min-w-64 text-sm font-semibold text-slate-800">
        ...
        >
          {workspaceMolecules.map((molecule) => (
            <option key={molecule.id} value={molecule.id}>{molecule.name} — {molecule.formula}</option>
          ))}
        </select>
      </label>
      <div className="flex flex-wrap gap-2">
        ...
        <button type="button" onClick={reset} className="rounded-lg border border-rose-200 px-4 py-2 text-sm font-semibold text-rose-700 hover:bg-rose-50">Reset workspace</button>
      </div>
    </div>
  );
}
